...
The Byte Pair Encoding (BPE) \citep{sennrich2016} is a method of preprocessing of input and output text enabling the Neural Machine Translation models to operate on subword units, and thus allowing them to handle words unseen during training. Since in NBA there is a fixed set of cities, teams and players (only about 10 players from development and 10 from test set were unknown from training) this isn't our main concern. However we can use BPE to lower the overall number of tokens in the output summaries, making them denser and allowing the neural network to possibly use richer vocabulary. I describe the algorithm, and show how it helped in the presented goals.

\subsubsection{Algorithm}

I use the implementation of the algorithm from the authors of the BPE paper\footnote{\url{https://github.com/rsennrich/subword-nmt}}, which is also downloadable as a standalone python package. The idea is to divide each token from the train set to characters, and add them to the special symbol vocabulary. Then we iterate over the text and merge together the most common pair of succeeding characters to create a new symbol added to the vocabulary. Next each occurrence of the pair is substitued by the new symbol. At the end (after $N$ iterations, where $N$ is the hyperparameter of the algorithm), the symbol vocabulary contains all the characters and newly created symbols.
