...
In the standard Encoder-Decoder, the output is a distribution over the output vocabulary. There are several flaws in the design:
...
The same author also states that the fact that the attentional decisions are made independently is suboptimal. Hence the \emph{Input Feeding} approach is proposed to allow the model to take into acount its previous decisions. It simply means that the next input is the concatenation $[y_t, att_t]$.
...
\begin{equation}
    p(y_t | o_{<t}, \mathbf{i}) = ( 1 - p^{switch}(z_t = 1)) * p^{vocab}(y_t) + p^{switch}(z_t = 1) * p^{copy}(y_t)
\end{equation}
...
\begin{equation}
    p^{copy}(y_t) = \sum_{i=1}^m{a_{t,i} * i_i}
\end{equation}

\begin{figure}
    \includegraphics[scale=0.85]{img/attention_alignment.pdf}
