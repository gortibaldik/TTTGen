\chapter{Experimental Set Up} \label{model_chapter}
...
\section{Truncated Backpropagation Through Time} \label{truncated_backprop_subsection}
...
\section{Baseline model} \label{sec_baseline_model_rotowire}
...
\begin{figure}[!h]
    \centering
    \includegraphics[scale=0.18]{img/rotowire_baseline.jpg}
...
\begin{figure}[!h]
    \centering
    \includegraphics[scale=0.35]{img/rotowire_joint_copy.jpg}
...
\section{Improving the Baseline with Copy Mechanisms}
...
As suggested, the Decoder is a one layer LSTM which operates on the \emph{context aware} representations $\hat{\mathbf{r}}^{cs}$. Its hidden states are initialized with $avg(\{\hat{r}_t^{cs}\}_{t=1}^m)$. \citep{puduppully2019datatotext} haven't elaborated on the exact approach, hence it is probable that I have diverged a little bit from the intentions of original authors.
