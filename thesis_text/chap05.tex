...
The models are trained to generate lowercased Byte Pair Encoded sequences. During the postprocessing we merge the tokens belonging to one word together (e.g. \emph{"contribu$\star$ tor"} $\rightarrow$ \emph{"contributor"}), split the tokens of named entities (e.g. \emph{"LeBron\_James"} $\rightarrow$ \emph{"LeBron James"}) and uppercase the first letter of a sentence. Since the vast majority of unique tokens (more than 98.6\%) from the validation and test sets (section \ref{section:stats_transformed_dataset}) is in the vocabulary collected from the training set, we opted to ommit 